...
export default function MoneyInForm({
  accounts = [],
  financeTaxonomy = {},
  form,
  setForm,
  ...
  return (
    <div className="grid">
      <section className="section">
        ...
        >
          <div className="form-grid">
            ...
            />
            <SourceModuleSelect value={form.module} onChange={(value) => setForm((f) => ({ ...f, module: value }))} />
            ...
            <label>
              Linked Receivable
              <select value={form.receivable_id} onChange={(e) => setForm((f) => ({ ...f, receivable_id: e.target.value }))}>
                <option value="">Select</option>
                {receivables.filter((r) => Number(r.balance_due || 0) > 0).map((row) => (
                  <option key={row.id} value={row.id}>#{row.id} · {row.counterparty_name} · Bal {row.balance_due}</option>
                ))}
              </select>
            </label>
            <ToggleField
      ...
      <JournalImpactPreview direction="in" amount={form.amount} paymentMethod={form.payment_method} />
    </div>
  );
}
