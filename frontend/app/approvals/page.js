...
export default function ApprovalsPage() {
  ...
  return (
    <div className="stack">
      ...
      <section className="section">
        <div className="tabs" role="group" aria-label="Approval queues" style={{ marginTop: 0 }}>
          {visibleTabs.map((name) => (
            <button key={name} type="button" aria-pressed={tab === name} className={tab === name ? 'tab active' : 'tab'} onClick={() => setTab(name)}>{TAB_LABELS[name]} ({counts[name] || 0})</button>
          ))}
        </div>
        {loading && <p className="small muted">Refreshing queues…</p>}
        {!loading && !visibleTabs.length && <p className="muted">No approval queues are available for your current permissions.</p>}
      </section>

      {tab === 'records' && visibleTabs.includes('records') && (
      ...
      {tab === 'procurement' && visibleTabs.includes('procurement') && (
      ...
      {tab === 'cashflow' && visibleTabs.includes('cashflow') && (
      ...
      {tab === 'journals' && visibleTabs.includes('journals') && (
      ...
      {tab === 'reconciliations' && visibleTabs.includes('reconciliations') && (
