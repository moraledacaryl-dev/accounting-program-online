...
const PRESETS = {
  'operating-expense': { module: 'finance', category: 'Cash', subcategory: 'Cash Movements', level3_item: 'Cash Out' },
  'supplier-payment': { module: 'procurement', category: 'Payables', subcategory: 'Supplier Payables', level3_item: 'Inventory Suppliers' },
  'utility-payment': { module: 'utilities', category: 'Payables', subcategory: 'Operational Payables', level3_item: 'Utility Payable' },
  'payroll-release': { module: 'payroll', category: 'Payables', subcategory: 'Payroll / Government Payables', level3_item: 'Payroll Payable' },
};
...
function MoneyOutContent() {
  ...
  async function load() {
    const [accountsData, taxData, payableData, txRows, templateRows] = await Promise.all([
      fetchFinancialAccounts({ only_active: true }),
      fetchModuleTaxonomy('finance'),
      fetchPayables({ limit: 300 }),
      fetchMoneyTransactions({ direction: 'out', start_date: todayISO(), end_date: todayISO(), limit: 200 }),
    ...
    setTemplates((Array.isArray(templateRows) ? templateRows : []).filter((t) => t.direction === 'out'));
  }

  useEffect(() => {
    load().catch((e) => setError(e.message || 'Failed to load money-out page.'));
  }, []);
  ...
  async function submit(e) {
    ...
    try {
      const payload = {
        ...
        payable_id: form.payable_id ? Number(form.payable_id) : null,
      ...
      if (!editingId && form.attachment_file && created?.id) {
        await uploadAttachment({
          ...
          entityId: created.id,
          note: form.notes || `Money-out attachment ${created.id}`,
        });
      }
      setNotice(editingId ? 'Money out record updated.' : 'Money out record saved.');
      setForm({ ...EMPTY_FORM, transaction_date: form.transaction_date });
      setEditingId(null);
      setTemplateId('');
      await load();
    } catch (err) {
      setError(err.message || 'Failed to save money out record.');
    }
  }
  ...
  function startEdit(row) {
    ...
    setForm({
      ...
      level3_item: row.level3_item || 'Cash Out',
      amount: row.amount ?? '',
      payment_method: row.payment_method || 'cash',
      reference_no: row.reference_no || '',
      counterparty_name: row.counterparty_name || '',
      notes: row.notes || '',
      payable_id: row.payable_id ? String(row.payable_id) : '',
  ...
  return (
    <div className="stack">
      ...
      <section className="section">
        <h1>Money Out</h1>
        <p className="muted">Record spending, link it to an open bill when needed, and attach proof if available.</p>
        {editingId ? <p className="small muted">Editing money out record #{editingId}</p> : null}
      ...
      <MoneyOutForm
        ...
        payables={payables}
        onSubmit={submit}
        submitLabel={editingId ? 'Update Money Out' : 'Save Money Out'}
      />
      ...
      <section className="section">
        ...
        <table className="table">
          ...
          <tbody>
            ...
            {!rows.length && <tr><td colSpan="9" className="muted">No money-out records for today.</td></tr>}
          </tbody>
        </table>
      </section>
    </div>
