...
def _standard_plan_rank(plan: RatePlan) -> tuple[int, int]:
    ...
    if code in _STANDARD_RATE_NAMES:
        return (0, int(getattr(plan, 'id', 0) or 0))
    if name in _STANDARD_RATE_NAMES:
        return (1, int(getattr(plan, 'id', 0) or 0))
    if any(code.startswith(f'{token}-') or code.startswith(f'{token}_') for token in _STANDARD_RATE_NAMES):
        return (2, int(getattr(plan, 'id', 0) or 0))
    if any(name.startswith(token) for token in _STANDARD_RATE_NAMES):
        return (3, int(getattr(plan, 'id', 0) or 0))
...
def reconciliation_channels(db: Session = Depends(get_db), user=Depends(require_any_permissions('bookings.view', 'cashflow.view', 'reports.view'))):
    ...
    return [{'id': row.id, 'code': row.code, 'name': row.name, 'channel_class': row.channel_class, 'settlement_mode': row.settlement_mode, 'default_commission_rate': float(row.default_commission_rate or 0), 'is_prepaid': bool(row.is_prepaid)} for row in rows]
...
def payout_bookings(channel_id: int = Query(ge=1), search: str = Query(default='', max_length=120), db: Session = Depends(get_db), user=Depends(require_any_permissions('bookings.view', 'cashflow.view', 'reports.view'))):
    query = (db.query(Booking, BookingChannel, ChannelPayoutBookingLink, ChannelPayout)
...
def reconciliation_history(channel_id: int | None = Query(default=None, ge=1), db: Session = Depends(get_db), user=Depends(require_any_permissions('bookings.view', 'cashflow.view', 'reports.view'))):
    query = (db.query(ChannelPayoutBookingLink, ChannelPayout, Booking, BookingChannel)
    ...
    try:
        ...
        for item in payload.items:
            ...
            if existing_link:
                ...
                existing_link = ChannelPayoutBookingLink(payout_id=payout.id, booking_id=booking.id, payout_reference=(payload.payout_reference or '').strip() or None)
